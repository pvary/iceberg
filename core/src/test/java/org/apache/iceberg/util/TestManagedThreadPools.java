...
class TestManagedThreadPools {

  @AfterEach
  void restoreThreadPoolsState() {
    ManagedThreadPools.init();
  }

  @Test
  void newExitingWorkerPoolShutdown() {
    ExecutorService pool = ManagedThreadPools.newExitingWorkerPool("test-exiting-pool", 2);

    ManagedThreadPools.shutdownThreadPools();
  ...
  void multipleExitingPoolsShutdown() {
    ExecutorService pool1 = ManagedThreadPools.newExitingWorkerPool("test-pool-1", 1);
    ExecutorService pool2 = ManagedThreadPools.newExitingWorkerPool("test-pool-2", 1);
  ...
  void exitingScheduledPoolShutdown() {
    ExecutorService scheduled =
        ManagedThreadPools.newExitingScheduledPool("test-scheduled", 1, Duration.ofSeconds(5));
  ...
  void shutdownThreadPoolsInterruptsRunningTasks() throws Exception {
    ...
    ExecutorService slowPool =
        ManagedThreadPools.newExitingScheduledPool("test-slow-pool", 1, Duration.ofMillis(50));
