...
import static org.apache.iceberg.actions.RewriteDataFiles.REWRITE_JOB_ORDER_DEFAULT;
...
public class RewriteFileGroupPlanner
  ...
  public RewriteFileGroupPlanner(Table table, Expression filter, long snapshotId) {
    super(table);
    this.filter = filter;
  ...
  CloseableIterable<FileScanTask> tasks() {
    return table().newScan().useSnapshot(snapshotId).filter(filter).ignoreResiduals().planFiles();
  }
