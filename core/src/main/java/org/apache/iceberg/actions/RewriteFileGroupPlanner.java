...
public class RewriteFileGroupPlanner {
   ...
   */
  public RewritePlan plan(Table table, Expression filter, long snapshotId) {
    StructLikeMap<List<List<FileScanTask>>> plan = planFileGroups(table, filter, snapshotId);
  ...
  private StructLikeMap<List<List<FileScanTask>>> planFileGroups(
      Table table, Expression filter, long snapshotId) {
    CloseableIterable<FileScanTask> fileScanTasks =
        table.newScan().useSnapshot(snapshotId).filter(filter).ignoreResiduals().planFiles();
